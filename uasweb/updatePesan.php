<?php
    ...
    $result = mysqli_query($conn, "SELECT * FROM daftarpesan WHERE id=$id");
    $pesanan = [];
    ...
    if (isset($_POST['kirim'])){
        ...
        if (move_uploaded_file($temp, 'img/'. $daftarpesan ."/" . $gambar_baru)){
            $query = mysqli_query($conn, "UPDATE daftarpesan SET nama = '$nama', nomorhp = '$nomorhp', alamat = '$alamat', barang = '$barang', panjang = '$panjang', tinggi = '$tinggi', metode = '$metode', gambar = '$gambar_baru', waktu = '$waktu' WHERE id = '$id'");
            if ($query) {
                echo "
                    <script>
            ...
            echo "
                <script>
                    ...
                    document.location.href = 'updatePesan.php';
                </script>
            ";
        }
    }
